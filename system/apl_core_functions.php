...
function aplVerifyPluginPurchase($LICENSE_CODE = "", $plugin_product_id = "", $domain_name = "", $action = "")
{
    $content_array = aplCustomPost(APL_ROOT_URL . "/apl_callbacks/verify_envato_purchase_plugin.php", "product_id=" . rawurlencode(APL_PRODUCT_ID) . "&license_code=" . rawurlencode($LICENSE_CODE) . "&plugin_product_id=" . rawurlencode($plugin_product_id) . "&domain_name=" . rawurlencode($domain_name) . "&action=" . rawurlencode($action) . "&connection_hash=" . rawurlencode(hash("sha256", "verify_envato_purchase")));

    if (!empty($content_array)) //response received
    {
        $response = json_decode($content_array['body']);
        return $response;
    }
...
function aplVerifyLicense($MYSQLI_LINK = null, $FORCE_VERIFICATION = 0)
{
    ...
    if (empty($apl_core_notifications = aplCheckSettings())) //only continue if script is properly configured
    {
        if (aplCheckData($MYSQLI_LINK)) //only continue if license is installed and properly configured
        {
            ...
            {
                if ($update_lcd_value == 1) //generate new $LCD value ONLY if verification succeeded. Otherwise, old $LCD va`lue should be used, so license will be verified again next time script is executed
                {
                    $LCD = date("Y-m-d");
...
//custom function to register client's email address, purchase code, and support validity
function aplCustomClientRegistration($LICENSE_CODE, $domain, $clientEmail)
{
    // check for valid Envato code (36 symbols, from which 4 are "-") was submitted
    if (strlen($LICENSE_CODE) == 36 && substr_count($LICENSE_CODE, "-") == 4) {
        aplCustomPost(APL_ROOT_URL . "/apl_callbacks/custom_user_register.php", "product_id=" . rawurlencode(APL_PRODUCT_ID) . "&license_code=" . rawurlencode($LICENSE_CODE) . "&domain=" . $domain . "&client_registered_email=" . $clientEmail);
    }
}